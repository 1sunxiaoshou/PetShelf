# PetShelf Full-Stack Design Specification
This document details the architecture, database schema, API contracts, and integration strategies required to upgrade the PetShelf application from a static frontend prototype into a complete, secure, full-stack production-ready web application.
---
## 1. System Architecture
The upgraded PetShelf system uses a robust **React + Vite + Express + Neon PostgreSQL** architecture. 
```mermaid
graph TD
    subgraph Client [Browser Client]
        React[React Frontend]
        ViteDev[Vite Dev Server / Proxy]
        AuthSDK[@neondatabase/neon-js/auth]
    end
    subgraph Backend [Node.js Backend]
        Express[Express.js Server]
        Multer[Multer File Upload]
        AdmZip[Adm-Zip Packer]
        PGDriver[pg node-postgres client]
    end
    subgraph External [Neon Cloud Services]
        NeonDB[(Neon PostgreSQL 17)]
        NeonAuth[Neon Auth Service]
    end
    React <-->|Proxy /api| ViteDev
    ViteDev <-->|HTTP Requests| Express
    React <-->|Login / Sessions| AuthSDK
    AuthSDK <-->|OAuth / OIDC| NeonAuth
    NeonAuth <-->|Sync Users/Sessions| NeonDB
    Express <-->|SQL Queries| PGDriver
    PGDriver <-->|Postgres connection| NeonDB
    Express -.->|Save spritesheets| LocalStorage[public/uploads/]
```
### Key Components:
1. **Frontend (Vite + React)**: Renders the user interface. Makes API calls to `/api/*` which are proxied to the Express server during development, or served directly in production.
2. **Backend (Express)**: Manages database connections, processes file uploads via `multer`, generates downloadable `.zip` archives via `adm-zip`, and executes route logic.
3. **Database (Neon PostgreSQL 17)**: Stores persistent data including pets, user profiles, likes, and download histories.
4. **Authentication (Neon Auth)**: Handles authentication, signup, signin, and secure token verification directly backed by PostgreSQL.
---
## 2. Database Schema
All database tables will be created in the Neon database. The tables are configured to map seamlessly to Neon Auth user IDs.
### 2.1 Table: `pets`
Stores metadata and paths for all uploaded pets.
| Field Name | Data Type | Constraints | Description |
| :--- | :--- | :--- | :--- |
| `id` | `UUID` | `PRIMARY KEY`, `DEFAULT gen_random_uuid()` | Unique internal identifier for the pet in the library. |
| `manifest_id` | `VARCHAR(100)` | `NOT NULL`, `UNIQUE` | Unique bundle identifier (from `pet.json` `id`). |
| `display_name` | `VARCHAR(100)` | `NOT NULL` | Human-readable name of the pet (from `pet.json` `displayName`). |
| `description` | `TEXT` | - | Short description of the pet (from `pet.json` `description`). |
| `author_id` | `VARCHAR(255)` | `NOT NULL` | The Neon Auth user ID of the creator who uploaded the pet. |
| `author_name` | `VARCHAR(100)` | `NOT NULL` | The username or email alias of the author (for display). |
| `spritesheet_path` | `TEXT` | `NOT NULL` | Relative path to the uploaded WebP file on the server (e.g., `/uploads/atri_spritesheet.webp`). |
| `downloads_count` | `INT` | `DEFAULT 0` | The total number of downloads. |
| `likes_count` | `INT` | `DEFAULT 0` | The total number of likes/stars. |
| `created_at` | `TIMESTAMPTZ` | `DEFAULT CURRENT_TIMESTAMP` | The timestamp when the pet was uploaded. |
| `updated_at` | `TIMESTAMPTZ` | `DEFAULT CURRENT_TIMESTAMP` | The timestamp when metadata was last updated. |
### 2.2 Table: `pet_likes`
Maintains the many-to-many relationship representing which users have liked which pets.
| Field Name | Data Type | Constraints | Description |
| :--- | :--- | :--- | :--- |
| `pet_id` | `UUID` | `REFERENCES pets(id) ON DELETE CASCADE` | The pet that was liked. |
| `user_id` | `VARCHAR(255)` | `NOT NULL` | The Neon Auth user ID who liked the pet. |
| `created_at` | `TIMESTAMPTZ` | `DEFAULT CURRENT_TIMESTAMP` | Timestamp of when the like occurred. |
| **Primary Key** | `(pet_id, user_id)` | | Restricts users to liking any single pet only once. |
---
## 3. Backend API Specification
All backend endpoints are prefixed with `/api`. Error handlers will return clear JSON payloads: `{ "error": "Error message" }`.
### 3.1 `GET /api/pets`
Fetches a list of pets, filtered by search query and sorted by options.
*   **Query Parameters**:
    *   `query` (optional): Filter pets by `display_name` or `author_name` (case-insensitive substring).
    *   `sort` (optional): `hot` (likes + downloads combined), `new` (created_at desc), `downloads` (downloads_count desc), `likes` (likes_count desc).
*   **Response (200 OK)**:
    ```json
    [
      {
        ...
        "manifestId": "atri-pet",
        "displayName": "Atri",
        "description": "亚特鲁的桌宠，活泼可爱",
        "author": "CodexCreator",
        "downloads": 42,
        "likes": 12,
        "spritesheetPath": "/uploads/atri_spritesheet.webp",
...
*   **Response (200 OK)**:
    ...
    ```
*   **Response (404 Not Found)**: `{ "error": "Pet not found" }`
### 3.3 `POST /api/pets` (Authentication Required)
Handles the uploading and validation of a new pet folder.
*   **Request Format**: `multipart/form-data`
    *   `manifest`: JSON string containing `id`, `displayName`, `description`.
    *   `spritesheet`: WebP or PNG binary file.
*   **Action**:
    1.  Validates user session through Neon Auth headers.
    2.  Verifies the size (under 10MB).
    3.  Stores the spritesheet inside backend `public/uploads/` with a unique name.
    4.  Inserts metadata into the `pets` database table.
*   **Response (201 Created)**:
    ```json
    {
      ...
      "pet": {
        "id": "new-uuid",
        "displayName": "New Pet Name"
      }
    }
    ```
*   **Response (400/401 Error)**: `{ "error": "Validation failed" }` or `{ "error": "Unauthorized" }`
### 3.4 `POST /api/pets/:id/like` (Authentication Required)
Toggles the like state for the current logged-in user on a pet.
*   **Action**:
    1.  Verifies the user identity from Neon Auth.
    2.  Checks if a row in `pet_likes` exists for `(pet_id, user_id)`.
    3.  If it exists: deletes the row, decrements `likes_count` on the pet, returns `{ "liked": false }`.
    4.  If it does not exist: inserts a row, increments `likes_count` on the pet, returns `{ "liked": true }`.
*   **Response (200 OK)**: `{ "success": true, "liked": true, "likesCount": 13 }`
### 3.5 `GET /api/pets/:id/download`
Increments download count and returns a dynamically packaged `.zip` containing the pet workspace.
*   **Action**:
    1.  Increments `downloads_count` in the database.
    2.  Reads the pet details from the database.
    3.  Pipes the spritesheet file and a newly constructed `pet.json` file into an in-memory zip archive using `adm-zip`.
    4.  Streams the zip file to the browser with headers: `Content-Disposition: attachment; filename="<manifest_id>.zip"`.
*   **Response (200 OK)**: Binary `.zip` download stream.
### 3.6 User Profile Data Routes (Authentication Required)
*   `GET /api/me/uploads`: Returns all pets uploaded by the current user.
*   `GET /api/me/likes`: Returns all pets liked by the current user.
---
## 4. Neon Auth & User Integration Plan
### Neon Auth Setup
Neon Auth runs a managed authentication server. We connect our frontend by defining the Auth URL.
1.  **Frontend Auth Client Initialization**:
    Create `src/utils/auth.js` to manage session storage:
    ```javascript
    ...
    ```
2.  **Login Action**:
    When the user clicks the Login button, trigger `authClient.login()` which redirects them to Neon's highly secure managed login/signup view. Upon successful login, Neon redirects back to PetShelf, and the SDK parses the session cookies/tokens automatically.
3.  **Backend Token Verification**:
    Every API request to private routes must include the token in the `Authorization: Bearer <token>` header or rely on session cookies. The Express backend will use Neon's token verifier or queries against the Auth schema to validate the user and retrieve their UUID and username.
---
## 5. Seed / Default Data Integration
To ensure the web application is fully populated out-of-the-box, we will implement a seeding mechanism on server startup:
*   **Target Seed**: The `public/pets/atri/` assets already present.
*   **Process**:
    1.  On server boot, verify if the `pets` table has 0 records.
    2.  If empty, scan `public/pets/atri/` to read `pet.json` and locate `spritesheet.webp`.
    3.  Copy `spritesheet.webp` into the `public/uploads/` directory as `atri_spritesheet.webp`.
    4.  Insert the initial `Atri` pet record with `manifest_id = 'atri-pet'`, `display_name = 'Atri'`, and creator set to a system seed account.
    5.  This ensures that the repository works instantly upon download and database setup.
---
## 6. Verification Plan
We will verify both frontend functionality and database integration using:
1.  **Backend Integration Tests**: Scripted SQL and route validation.
2.  **Manual Verification**: Use browser-driven interaction to perform login, upload a custom pet folder, verify the validation checklist succeeds, download the resulting zip file, like the pet, and review the personal account panel updating correctly.
